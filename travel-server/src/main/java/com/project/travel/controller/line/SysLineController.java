...
public class SysLineController {
    ...
    public Result getSysLinePage(@RequestBody SysLine sysLine) {
        ...
        queryWrapper.lambda()
                ...
                .orderByDesc(SysLine::getTemperature);
    ...
    /** 增加点击量/热度 */
    @GetMapping("addLineHits")
    public Result addLineHits(@RequestParam("id")String id) {
        SysLine line = sysLineService.getById(id);
        if (line != null) {
            if (line.getTemperature() == null) {
                line.setTemperature(0);
            }
            line.setTemperature(line.getTemperature() + 1);
            sysLineService.updateById(line);
        }

...
public class SysAttractionOrderController {
    ...
    public Result getSysAttractionOrderPage(@RequestBody SysAttractionOrder sysAttractionOrder) {
        ...
        queryWrapper.lambda()
                ...
                .orderByDesc(SysAttractionOrder::getCreateTime);
        Page<SysAttractionOrder> sysAttractionOrderPage = sysAttractionOrderService.page(page, queryWrapper);
        
        // 补充景点信息，确保即使是旧订单也能正常显示
        if (sysAttractionOrderPage.getRecords() != null && !sysAttractionOrderPage.getRecords().isEmpty()) {
            for (SysAttractionOrder order : sysAttractionOrderPage.getRecords()) {
                if (order.getAttractionsId() != null && (order.getName() == null || order.getImages() == null)) {
                    SysAttractions attractions = sysAttractionsService.getById(order.getAttractionsId());
                    if (attractions != null) {
                        order.setName(attractions.getName());
                        order.setIntroduce(attractions.getIntroduce());
                        order.setImages(attractions.getImages());
                    }
                }
            }
        }

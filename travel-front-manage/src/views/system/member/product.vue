      ...
      <div class="search">
        <el-row :gutter="10" style="padding:10px">
          ...
          <el-col :xs="24" :sm="12" :md="8" :lg="8" :xl="8">
            ...
            <el-select style="margin-top:10px;width:100%" size="mini" v-model="search.type" clearable placeholder="全部">
        ...
        <el-table v-loading="loading" :data="tableData" @selection-change="handleSelectionChange" stripe style="width: 100%">
          ...
          <el-table-column label="图片" width="90">
            <template slot-scope="scope">
              <img v-if="scope.row.images" :src="scope.row.images.split(',')[0]" class="product-thumb">
            </template>
          </el-table-column>
          <el-table-column prop="name" label="名称" min-width="120"></el-table-column>
          <el-table-column prop="pointsRequired" label="所需积分" width="90"></el-table-column>
          <el-table-column prop="price" label="参考价(元)" width="100"></el-table-column>
          <el-table-column label="类型" width="90">
            <template slot-scope="scope">
              {{ scope.row.type === 0 ? '实物' : scope.row.type === 1 ? '纪念品' : '虚拟' }}
            </template>
          </el-table-column>
...
<script>
import { getSysProductPage, removeSysProduct, toggleProductStatus } from '../../../api/api'
import add from '../../../components/system/product/addProduct'
import update from '../../../components/system/product/updateProduct'
...
export default {
  ...
  methods: {
    ...
    query() {
      getSysProductPage(this.search).then(res => {
        if (res.code == 1000) {
    ...
    toggleStatus(row, status) {
      toggleProductStatus({ id: row.id, status }).then(res => {
        if (res.code == 1000) {
          this.query()
        } else {
          this.$notify.error({ title: '错误', message: res.message })
